...
volatile uint16_t lockState = 1; //锁状态，为1时锁为开锁状态，led灯灭，为0时锁为上锁状态，led点亮
volatile uint16_t syncFlag  = 0; //同步状态，为1时需要同步，为0时不需要同步
/*******************************************************************************
 * Code
 ******************************************************************************/
...
void setLock(uint32_t state)
{
  if(state == 0)  //如果状态为0，表示上锁，这里用LED点亮表示。
  {
    GPIO_PinWrite(GPIO, APP_BOARD_RED_LED_PORT,   APP_BOARD_RED_LED_PIN,   0);//LED为负逻辑，0表示点亮
  }else   //如果状态不为0，表示开锁，这里用LED熄灭表示
  {
